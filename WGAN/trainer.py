...
class WGANTrainer():
    ...
    def gen_step(self, z):
        ...
        # WGAN generator objective: push the critic to score its fakes as high as possible.
        # Sign flip converts the "maximize critic on fakes" into a minimization for the optimizer.
        self.optimizer_gen.zero_grad()
        fake_images = self.model_gen(z)
        loss_gen = -self.model_disc(fake_images).mean()
        loss_gen.backward()
        self.optimizer_gen.step()

        self.gen_loss_log.append(loss_gen.item())

    def disc_step(self, real_images, z):
        '''
        ...
        # WGAN critic objective: maximize D(real) - D(fake), i.e. minimize D(fake) - D(real).
        # The generator output is detached so its computation graph is not retained for this step.
        self.optimizer_disc.zero_grad()
        fake_images = self.model_gen(z).detach()
        loss_disc = self.model_disc(fake_images).mean() - self.model_disc(real_images).mean()
        loss_disc.backward()
        self.optimizer_disc.step()
        # Clamping enforces the K-Lipschitz constraint required by Theorem 3 of the WGAN paper.
        self.clamp_weights()
